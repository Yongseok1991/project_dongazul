...
public class ProfileController {
	...
	public void infoThem(
		...
		log.debug("infoThem(email, rttrs, model) invoked.");
		
		ProfileDTO profile = this.service.getProfile(email);
		model.addAttribute(profileKey, profile);
	} // infoGet
	
	 @PostMapping("/infoThem")
	   public void infoThemPost(
	         String email,
	         RedirectAttributes rttrs,
	         Model model
	         ) throws Exception {
	      
	      log.debug("infoThemPost(email, rttrs, model) invoked.");
	      
	      service.likesPlus(email);
